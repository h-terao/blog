...
ここで、PCを一度再起動しましょう。再起動完了後、`rocm-smi`コマンドを叩いてみて以下のような画面になれば、インストール成功です。
...
次に、[ROCm/jax](https://github.com/ROCm/jax)をクローンして、インストールしたいバージョンのタグに切り替えます。
（今回は、v0.4.30をインストールしていきます。）
```bash
